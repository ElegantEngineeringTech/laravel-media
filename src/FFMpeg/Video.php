...
class Video extends FFMpeg
{
    ...
    public function hls(
        ...
        $streams = parent::metadata($input)['streams'] ?? [];
        $hasAudio = false;
        ...
        $variants = $variants
            ->setMaxBitrate($sourceVideoBitrate)
            ->setMaxAudioBitrate($sourceAudioBitrate);
        ...
        foreach ($variants as $index => $variant) {
            $arguments = [
                ...$arguments,
                "-b:v:{$index}", $variant->bitrate->format(),
                "-maxrate:v:{$index}", $variant->maxrate->format(),
                "-bufsize:v:{$index}", $variant->bufsize->format(),
            ];
        }
        ...
        if ($hasAudio) {
            ...
            foreach ($variants as $index => $variant) {
                $arguments = [
                    ...$arguments,
                    "-b:a:{$index}", $variant->audioBitrate->format(),
                ];
            }
        } else {
            $arguments[] = '-an';
